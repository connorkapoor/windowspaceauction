...
export async function getBids() {
  try {
    if (isBlobAvailable()) {
      ...
      const blobs = await list({ prefix: BIDS_BLOB_KEY });
      const bidsBlob = blobs.blobs.find(blob => blob.pathname === BIDS_BLOB_KEY);
      
      if (bidsBlob) {
        // Fetch the bids from the blob with a cache-busting query parameter
        const response = await fetch(`${bidsBlob.url}?t=${Date.now()}`);
        if (response.ok) {
...
export async function addBid(bid) {
  try {
    ...
    const currentBids = await getBids();
    ...
    // Append the new bid to the current bids
    const updatedBids = [...currentBids, newBid];
    
    if (isBlobAvailable()) {
      // Store the updated bids in the blob
      // Use overwrite: true to ensure we're replacing any existing blob
      ...
      // For debugging: log the bids that were stored
      console.log('Stored bids:', updatedBids);
    } else {
...
export async function getAuctionSettings() {
  try {
    if (isBlobAvailable()) {
      ...
      if (settingsBlob) {
        // Fetch the settings from the blob with a cache-busting query parameter
